...
echo
echo "== Validating reconciliation evidence receipts dry-run =="
python scripts/build_reconciliation_receipts.py --dry-run
...
echo
echo "== Removing ignored reconciliation evidence receipt artifacts =="
if [ -d outputs/reconciliation_evidence_receipts ]; then
  git clean -fX outputs/reconciliation_evidence_receipts/
fi
