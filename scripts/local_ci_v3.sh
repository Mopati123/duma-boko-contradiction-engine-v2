...
echo
echo "== Validating reconciliation hash chain dry-run =="
python scripts/build_reconciliation_hash_chain.py --dry-run
...
echo
echo "== Removing ignored reconciliation hash chain artifacts =="
if [ -d outputs/reconciliation_hash_chain ]; then
  git clean -fX outputs/reconciliation_hash_chain/
fi
